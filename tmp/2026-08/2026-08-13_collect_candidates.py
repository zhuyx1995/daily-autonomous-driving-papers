...
OUT_DIR = ROOT / 'tmp' / '2026-08' / '2026-08-13'
OUT_DIR.mkdir(parents=True, exist_ok=True)
UA = 'daily-autonomous-driving-papers/1.8 (local automation)'
...
def fetch_submitted_v1(aid: str):
    ...
    if not m:
        m = re.search(r'Submitted on\s+([0-9]{1,2}\s+[A-Za-z]{3}\s+[0-9]{4})', html_text, re.I)
        if m:
            raw = m.group(1)
            try:
    ...
    if dt is None:
        ...
        try:
            dt = datetime.strptime(raw2, '%d %b %Y %H:%M:%S %Z').replace(tzinfo=timezone.utc)
        except Exception:
            try:
                dt = datetime.strptime(raw2.split(' GMT')[0], '%d %b %Y %H:%M:%S').replace(tzinfo=timezone.utc)
            except Exception:
                return None, raw
    return dt, raw
...
for md in sorted((ROOT / 'reports').rglob('*.md')):
    ...
    for pid in re.findall(r'<!--\s*PAPER:\s*arxiv-([0-9]{4}\.[0-9]{4,5})\s+START\s*-->', txt, re.I):
        history_ids.add(pid)
    for title in re.findall(r'^##\s+\d+\.\s+(.+)$', txt, re.M):
        history_titles.add(norm_title(title))
...
for aid, row in sorted(raw_candidates.items()):
    ...
    if aid in history_ids or row['nt'] in history_titles:
        continue
    dt, raw_v1 = fetch_submitted_v1(aid)
    time.sleep(1.3)
    if not dt:
        continue
    sub_date = dt.date()
    if not (WINDOW_START <= sub_date <= TODAY):
        continue
    rows.append((aid, row['title'], sub_date.isoformat(), dt.isoformat().replace('+00:00', 'Z'), raw_v1 or '', str(row['score']), row['cats'], row['source'], row['summary'], row['nt']))

rows.sort(key=lambda r: (r[2], r[3], int(r[5]), r[0]), reverse=True)
with (OUT_DIR / 'recent_candidates.tsv').open('w', encoding='utf-8') as f:
    f.write('aid\ttitle\tdate\tv1_utc\tv1_raw\tscore\tcats\tsource\tsummary\tnorm_title\n')
    for row in rows:
        f.write('\t'.join(row) + '\n')

print(f'window={WINDOW_START}..{TODAY}')
print(f'history_ids={len(history_ids)} history_titles={len(history_titles)}')
print(f'raw_candidates={len(raw_candidates)} filtered_recent_new={len(rows)}')
for row in rows[:40]:
    print(f'{row[2]}\t{row[5]}\t{row[0]}\t{row[1]}')
